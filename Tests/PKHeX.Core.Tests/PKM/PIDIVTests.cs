...
namespace PKHeX.Tests.PKM
{
    public class PIDIVTest
    {
        ...
        {
            // Method 1/2/4
            PK3? pk1 = new PK3 {PID = 0xE97E0000, IVs = new[] {17, 19, 20, 16, 13, 12}};
            PIDIV? ga1 = MethodFinder.Analyze(pk1);
            Assert.Equal(PIDType.Method_1, ga1.Type);
            PK3? pk2 = new PK3 {PID = 0x5271E97E, IVs = new[] {02, 18, 03, 12, 22, 24}};
            Assert.Equal(PIDType.Method_2, MethodFinder.Analyze(pk2).Type);
            PK3? pk4 = new PK3 {PID = 0x31B05271, IVs = new[] {02, 18, 03, 05, 30, 11}};
        ...
        public void PIDIVMatchingTest3Unown()
        {
            ...
            PK3? pk1U = new PK3 {PID = 0x815549A2, IVs = new[] {02, 26, 30, 30, 11, 26}, Species = 201}; // Unown-C
            Assert.Equal(PIDType.Method_1_Unown, MethodFinder.Analyze(pk1U).Type);
            PK3? pk2U = new PK3 {PID = 0x8A7B5190, IVs = new[] {14, 02, 21, 30, 29, 15}, Species = 201}; // Unown-M
            Assert.Equal(PIDType.Method_2_Unown, MethodFinder.Analyze(pk2U).Type);
            PK3? pk4U = new PK3 {PID = 0x5FA80D70, IVs = new[] {02, 06, 03, 26, 04, 19}, Species = 201}; // Unown-A
        ...
        public void PIDIVMatchingTest3Method3()
        {
            ...
            PK3? m3R = new PK3 { PID = 0x3DD1BB49, IVs = new[] { 23, 12, 31, 09, 03, 03 }, Species = 001 }; // Regular
            PIDType m3t = MethodFinder.Analyze(m3R).Type;
            Assert.Equal(PIDType.Method_3, m3t);
            PK3? m3u = new PK3 { PID = 0xBB493DD1, IVs = new[] { 23, 12, 31, 09, 03, 03 }, Species = 201 }; // Unown
            PIDType u3t = MethodFinder.Analyze(m3u).Type;
        ...
        {
            // Colosseum / XD
            PK3? pk3 = new PK3 {PID = 0x0985A297, IVs = new[] {06, 01, 00, 07, 17, 07}};
            PIDIV? ak3 = MethodFinder.Analyze(pk3);
            ...
            // Channel Jirachi
            PK3? pkC = new PK3 {PID = 0x264750D9, IVs = new[] {06, 31, 14, 27, 05, 27}, SID = 45819, OT_Gender = 1, Version = (int)GameVersion.R};
            PIDIV? akC = MethodFinder.Analyze(pkC);
        ...
        public void PIDIVMatchingTest3Event()
        {
            ...
            PK3? pkR = new PK3 {PID = 0x0000E97E, IVs = new[] {17, 19, 20, 16, 13, 12}};
            ...
            PK3? pkRA = new PK3 {PID = 0x0000E980, IVs = new[] {17, 19, 20, 16, 13, 12}, TID = 01337, SID = 60486};
            Assert.Equal(PIDType.BACD_R_A, MethodFinder.Analyze(pkRA).Type);

            // Unrestricted: TID/SID are zero.
            PK3? pkU = new PK3 {PID = 0x67DBFC33, IVs = new[] {12, 25, 27, 30, 02, 31}};
            ...
            PK3? pkUA = new PK3 {PID = 0x67DBFC38, IVs = new[] {12, 25, 27, 30, 02, 31}, TID = 01337, SID = 40657};
            Assert.Equal(PIDType.BACD_U_A, MethodFinder.Analyze(pkUA).Type);

            // berry fix zigzagoon: seed 0x0020
            PK3? pkRS = new PK3 {PID = 0x38CA4EA0, IVs = new[] {00, 20, 28, 11, 19, 00}, TID = 30317, SID = 00000};
            PIDIV? a_pkRS = MethodFinder.Analyze(pkRS);
            ...
            PK3? gkRS = new PK3 { TID = 30317, SID = 00000 };
            ...
            PK3? nyxUA = new PK3 {PID = 0xBD3DF676, IVs = new[] {00, 15, 05, 04, 21, 05}, TID = 80, SID = 0};
            PIDIV? nyx_pkUA = MethodFinder.Analyze(nyxUA);
        ...
        public void PIDIVMatchingTest4()
        {
            ...
            PK4? pkCC = new PK4 {PID = 0x00000037, IVs = new[] {16, 13, 12, 02, 18, 03}, Species = 1, Gender = 0};
            ...
            PK4? pkASR = new PK4 {PID = 0x07578CB7, IVs = new[] {16, 13, 12, 02, 18, 03}};
            Assert.Equal(PIDType.G4MGAntiShiny, MethodFinder.Analyze(pkASR).Type);

            // Chain Shiny: TID/SID are zero.
            PK4? pkCS = new PK4 {PID = 0xA9C1A9C6, IVs = new[] {22, 14, 23, 24, 11, 04}};
        ...
        public void PIDIVMatchingTest5()
        {
            ...
            PK5? pkS5 = new PK5 {PID = 0xBEEF0037, TID = 01337, SID = 48097};
        ...
        {
            // Modest Method 1
            PK3? pk = new PK3 {PID = 0x6937DA48, IVs = new[] {31, 31, 31, 31, 31, 31}};
            ...
            // Test for Method J
            {
                ...
                Frame[]? results = FrameFinder.GetFrames(pidiv, pk).ToArray();

                System.Collections.Generic.IEnumerable<Frame>? failSync = results.Where(z => z.Lead == LeadRequired.SynchronizeFail);
                System.Collections.Generic.IEnumerable<Frame>? noSync = results.Where(z => z.Lead == LeadRequired.None);
                System.Collections.Generic.IEnumerable<Frame>? sync = results.Where(z => z.Lead == LeadRequired.Synchronize);
                ...
                int[]? slots = new[] { 0, 1, 2, 3, 4, 5, 6, 7, 9 };
                Assert.True(slots.All(s => results.Any(z => z.GetSlot(type) == s)), "Required slots not present.");
                IOrderedEnumerable<int>? slotsForType = results.Where(z => !z.LevelSlotModified).Select(z => z.GetSlot(type)).Distinct().OrderBy(z => z);
        ...
        {
            PK3? pk4 = new PK3 { PID = 0xFEE73213, IVs = new[] { 03, 29, 23, 30, 28, 24 } };
            PIDIV? analysis = MethodFinder.Analyze(pk4);
        ...
        public void PIDIVSearchEuclid()
        {
            ...
            System.Collections.Generic.IEnumerable<uint>? pidseeds = MethodFinder.GetSeedsFromPIDEuclid(RNG.XDRNG, rand0 >> 16,            rand1 >> 16);
            System.Collections.Generic.IEnumerable<uint>? ivseeds = MethodFinder.GetSeedsFromIVsEuclid(RNG.XDRNG, (rand0 >> 16) & 0x7FFF, (rand1 >> 16) & 0x7FFF);
