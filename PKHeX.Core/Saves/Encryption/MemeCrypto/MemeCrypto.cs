...
namespace PKHeX.Core
{
    ...
    public static class MemeCrypto
    {
        ...
        public static bool VerifyMemePOKE(byte[] input, out byte[] output)
        {
            ...
            int memeLen = input.Length - 8;
            MemeKeyIndex memeIndex = MemeKeyIndex.PokedexAndSaveFile;
            for (int i = input.Length - 8; i >= 0; i--)
            {
                if (BitConverter.ToUInt32(input, i) != POKE)
                    continue;

                int keyIndex = BitConverter.ToInt32(input, i + 4);
            ...
            foreach (int len in new[] { memeLen, memeLen - 2 }) // Account for Pokédex QR Edge case
            {
        ...
        public static bool VerifyMemeData(byte[] input, out byte[] output, MemeKeyIndex keyIndex)
        {
            ...
            MemeKey? key = new MemeKey(keyIndex);
            output = (byte[])input.Clone();

            byte[]? sigBuffer = key.RsaPublic(input.AsSpan(input.Length - 0x60));
        ...
        private static bool DecryptCompare(byte[] output, ReadOnlySpan<byte> sigBuffer, MemeKey key, SHA1 sha1)
        {
            ...
            byte[]? hash = sha1.ComputeHash(output, 0, output.Length - 0x8);
            ulong computed = BitConverter.ToUInt64(hash, 0);
            ulong existing = BitConverter.ToUInt64(output, output.Length - 0x8);
        ...
        public static bool VerifyMemeData(byte[] input, out byte[] output, int offset, int length)
        {
            ...
            {
                byte[]? newOutput = (byte[])input.Clone();
        ...
        public static byte[] Resign7(byte[] sav7)
        {
            ...
            bool isUSUM = sav7.Length == SaveUtil.SIZE_G7USUM;
            int ChecksumTableOffset = sav7.Length - 0x200;
            int MemeCryptoOffset = isUSUM ? 0x6C100 : 0x6BB00;
            int ChecksumSignatureLength = isUSUM ? 0x150 : 0x140;
            const int MemeCryptoSignatureLength = 0x80;

            byte[]? result = (byte[])sav7.Clone();

            // Store current signature
            byte[]? oldSig = sav7.Slice(MemeCryptoOffset, MemeCryptoSignatureLength);

            using SHA256? sha256 = SHA256.Create();
            byte[]? newSig = sha256.ComputeHash(sav7, ChecksumTableOffset, ChecksumSignatureLength);
            Array.Resize(ref newSig, MemeCryptoSignatureLength);

            if (VerifyMemeData(oldSig, out byte[]? memeSig, MemeKeyIndex.PokedexAndSaveFile))
