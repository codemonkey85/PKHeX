...
namespace PKHeX.Core
{
    ...
    public static class MoveSetApplicator
    {
        ...
        public static int[] GetMoveSet(this PKM pk, bool random = false)
        {
            ...
            int[]? moves = la.GetMoveSet(random);
            ...
            LegalityAnalysis? newLa = new LegalityAnalysis(clone);
        ...
        public static int[] GetMoveSet(this LegalityAnalysis la, bool random = false)
        {
            ...
            int[]? learn = la.GetSuggestedMovesAndRelearn();
        ...
        public static IReadOnlyList<int> GetSuggestedRelearnMoves(this LegalityAnalysis legal, IEncounterable? enc = null)
        {
            ...
            IReadOnlyList<int>? m = legal.GetSuggestedRelearnMovesFromEncounter(enc);
            if (m.Any(z => z != 0))
                return m;
            ...
            {
                CheckMoveResult[]? moves = legal.Info.Moves;
                for (int i = 0; i < moves.Length; i++)
                {
                    if (!moves[i].ShouldBeInRelearnMoves())
                        continue;

                    int move = legal.pkm.GetMove(i);
                    if (dn.CanBeDexNavMove(move))
                        return new[] { move, 0, 0, 0 };
                }
            }

            EncounterSuggestionData? encounter = EncounterSuggestion.GetSuggestedMetInfo(legal.pkm);
