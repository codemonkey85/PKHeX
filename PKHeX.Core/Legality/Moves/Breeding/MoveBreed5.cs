...
namespace PKHeX.Core
{
    ...
    public static class MoveBreed5
    {
        ...
        public static EggSource5[] Validate(int species, GameVersion version, int[] moves, out bool valid)
        {
            ...
            int[]? egg = Legal.EggMovesBW[species].Moves;

            BreedInfo<EggSource5> value = new BreedInfo<EggSource5>(count, learnset, moves, level);
        ...
        {
            int count = 0;
            foreach (EggSource5 x in info.Actual)
            {
                if (x == Base)
                    count++;
                else
        ...
        private static void MarkMovesForOrigin(in BreedInfo<EggSource5> value, ICollection<int> eggMoves, int count, bool inheritLevelUp, PersonalInfo info)
        {
            ...
            bool[]? tm = info.TMHM;
            Span<int> tmlist = Legal.TMHM_BW.AsSpan(0, 95); // actually 96, but TM96 is unavailable (Snarl - Lock Capsule)
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                int tmIndex = tmlist.IndexOf(move);
