    ...
    public static class MoveTutor
    {
        ...
        private static GameVersion GetIsTutor2(PKM pkm, int species, int move)
        {
            if (!ParseSettings.AllowGen2Crystal(pkm))
                return NONE;
            PersonalInfo? info = PersonalTable.C[species];
            int tutor = Array.IndexOf(Tutors_GSC, move);
        ...
        private static GameVersion GetIsTutor3(int species, int move)
        {
            // E Tutors (Free)
            // E Tutors (BP)
            PersonalInfo? info = PersonalTable.E[species];
            int e = Array.IndexOf(Tutor_E, move);
            ...
            int frlg = Array.IndexOf(SpecialTutors_FRLG, move);
            if (frlg != -1 && SpecialTutors_Compatibility_FRLG[frlg] == species)
                return GameVersion.FRLG;

            // XD
            int xd = Array.IndexOf(SpecialTutors_XD_Exclusive, move);
        ...
        {
            PersonalInfo? pi = PersonalTable.HGSS.GetFormEntry(species, form);
            int type = Array.IndexOf(Tutors_4, move);
            if (type != -1 && pi.TypeTutors[type])
                return GameVersion.Gen4;

            int special = Array.IndexOf(SpecialTutors_4, move);
        ...
        private static GameVersion GetIsTutor5(PKM pkm, int species, int form, bool specialTutors, int move)
        {
            ...
            int type = Array.IndexOf(TypeTutor6, move);
            if (type != -1 && pi.TypeTutors[type])
                    return GameVersion.Gen5;

            if (specialTutors && pkm.HasVisitedB2W2(species))
            {
                int[][]? tutors = Tutors_B2W2;
                for (int i = 0; i < tutors.Length; i++)
                {
                    int tutor = Array.IndexOf(tutors[i], move);
                    if (tutor == -1)
                        continue;
        ...
        private static GameVersion GetIsTutor7(PKM pkm, int species, int form, bool specialTutors, int move)
        {
            ...
            {
                int tutor = Array.IndexOf(Tutors_USUM, move);
        ...
        {
            PersonalInfoSWSH? pi = (PersonalInfoSWSH)PersonalTable.SWSH.GetFormEntry(species, form);
            int type = Array.IndexOf(TypeTutor8, move);
            if (type != -1 && pi.TypeTutors[type])
            ...
            int tutor = Array.IndexOf(Tutors_SWSH_1, move);
            if (tutor != -1 && pi.SpecialTutors[0][tutor])
                return GameVersion.USUM;
        ...
        private static void AddMovesTutor2(List<int> moves, int species, int format, bool korean = false)
        {
            if (korean)
                return;
            PersonalInfo? pi = PersonalTable.C[species];
        ...
        private static void AddMovesTutor3(List<int> moves, int species)
        {
            // E Tutors (Free)
            // E Tutors (BP)
            PersonalInfo? pi = PersonalTable.E[species];
